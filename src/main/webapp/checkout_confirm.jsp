...
<div id="ccShopCartWrapper">
    
    <ul id="cart-table-checkout" class="clearfix">
    <li class="title">
        <div class="items"><%= lb.get("items" + lang) %></div>
        <div class="price"><%= lb.get("price" + lang) %></div>
        <div class="qty"><%= lb.get("qty" + lang) %></div>
        <div class="total"><%= lb.get("totalHeader" + lang) %></div>
    </li>
    ...
    %>  
        <li class="product-line">
            ...
            <div class="product-qty"><%= product.getQuantity() %></div>
    ...
    <div class="clearfix">
    <div id="checkoutSubtotal"><%= lb.get("subtotal" + lang)%>: <%= SwissKnife.formatNumber(orderPrice.getGrossCurr1(), localeLanguage, localeCountry, minCurr1DispFractionDigits, curr1DisplayScale)%> &euro;</div>
    <div id="checkoutShipCost"><%= lb.get("shipCost" + lang)%>: <%= SwissKnife.formatNumber(shippingPrice.getGrossCurr1(), localeLanguage, localeCountry, minCurr1DispFractionDigits, curr1DisplayScale)%> &euro;</div>
    
    <div class="cart-total-checkout"><h2><%= lb.get("total" + lang) %>:</h2>&nbsp;&nbsp;<h2 class="shopcart-totalnum"><%= SwissKnife.formatNumber(orderPrice.getGrossCurr1().add(shippingPrice.getGrossCurr1()),localeLanguage,localeCountry,minCurr1DispFractionDigits,curr1DisplayScale) %> &euro;</h2></div>
    </div>
    ...
    while (availShipping != null && index < availShipping.length) {%>
        <div class="clearfix item<%if (order.getShippingWay().equals(availShipping[index][0])){%> selected<%}%>">
            <div class="checkbox"><input id="delivery_type_<%=availShipping[index][0]%>" style="margin:3px 15px 3px 10px;" type="radio" name="shippingWay" value="<%=availShipping[index][0]%>" <% if (order.getShippingWay().equals(availShipping[index][0])) out.print("checked"); else { %>onclick='document.shippingMethodForm.submit();'<% } %> /></div>
            <div class="info"><label for="delivery_type_<%=availShipping[index][0]%>"><%=availShipping[index][2]%> <span class="price"><%=SwissKnife.formatNumber(new BigDecimal(availShipping[index][3]),localeLanguage,localeCountry,minCurr1DispFractionDigits,curr1DisplayScale)%> &euro;</span></label></div>
        </div>
    <%
        index++;
    } %>
...
<div id="ccCustomerInfoWrapper">
    
    <div id="customerInfo">
    <div style="margin:0 0 10px 0;" class="clearfix">
    <span style="float:left; margin-top:10px; font-weight:bold;"><%= lb.get("customerInfo" + lang)%></span>
   ...
   <%
    if (customer.getBillingName().length()>0 && customer.getBillingAfm().length()>0) { %>
        <div id="invoiceInfo">
        <div style="margin:0 0 10px 0;" class="clearfix">
        <span style="float:left; margin-top:10px; font-weight:bold;"><%= lb.get("invoiceInfo" + lang) %></span>
        <div style="float:right;"><a href="<%= HTTP_PROTOCOL + serverName + "/" + response.encodeURL("checkout_billing.jsp") %>"><span class="button aux"><%= lb.get("change" + lang)%></span></a></div>
        </div>
    ...
    <div id="shippingInfo">
    <div style="margin:0 0 10px 0;" class="clearfix">
    <span style="float:left; margin-top:10px; font-weight:bold;"><%= lb.get("shippingInfo" + lang) %></span>
